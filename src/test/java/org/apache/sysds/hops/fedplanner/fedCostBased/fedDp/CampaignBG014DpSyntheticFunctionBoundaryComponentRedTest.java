...
/** Regression for exact formal occurrences that share one synthetic function-input boundary. */
@net.jcip.annotations.NotThreadSafe
public class CampaignBG014DpSyntheticFunctionBoundaryComponentRedTest {
	@Test
	public void twoWorkerStepLmKeepsSharedFormalPlacementsCoherent() throws Exception {
		...
		Path root = Path.of("target/g014-dp-synthetic-function-boundary");
		try {
			Files.createDirectories(root);
			Path features1 = matrixMetadata(root.resolve("features-1.data"), 25000, 2100, 52500000);
			Path features2 = matrixMetadata(root.resolve("features-2.data"), 25000, 2100, 52500000);
			...
			Files.writeString(script, stepLmScript(port1, port2, features1, features2,
				labels1, labels2, root.resolve("result.csv")));
			...
			Assert.assertTrue("DP must compile one coherent state for every formal sharing a function boundary",
				success);
		}
		finally {
	...
	private static String stepLmScript(int port1, int port2, Path features1, Path features2,
	...
	private static Path matrixMetadata(Path data, long rows, long cols, long nnz) throws Exception {
		...
		Files.writeString(Path.of(data + ".mtd"), "{\"data_type\":\"matrix\",\"value_type\":\"double\","

...
public class MinStExactPhysicalModelCertificateTest {
	private static final MinStExactCategoricalSolver.Limits CAMPAIGN_LIMITS =
	...
	public void sevenWorkloadsBuildBaselineFreePhysicalDomainsAndFactors() throws Exception {
		for(Workload workload : List.of(
			...
			MinStExactCategoricalSolver.Statistics statistics = model.analyze(CAMPAIGN_LIMITS);
			MinStExactCategoricalSolver.Result feasible = model.solveLegalityOnly(CAMPAIGN_LIMITS);
			MinStExactPhysicalModel.PhysicalSelection physical = model.physicalSelection(feasible);
			...
			MinStExactPhysicalSelection exact = MinStExactPhysicalSelection.create(model, optimized);
			var projected = MinStExactPhysicalPlacementProjector.project(exact);
			...
			for(var domain : model.domains())
				for(var alternative : domain.alternatives())
					if(alternative.authorityKind() == MinStExactPhysicalModel.AuthorityKind.LEGAL_SINGLETON) {
						...
						== MinStExactPhysicalModel.AuthorityKind.DURABLE_ANCHOR) {
	...
	public void nativeLocalInputDoesNotConstrainProducerPlacement() {
		var nativeLocal = new MinStExactPhysicalModel.InputAuthority(0,
			MinStExactPhysicalModel.InputAuthorityKind.NATIVE_LOCAL, null, null, null);
		Assert.assertTrue(MinStExactPhysicalModel.inputAuthorityPlacementSatisfied(nativeLocal,
	...
	public void conjunctiveForbidPairMatchesExactSelectorSemantics() throws Exception {
		...
		Assert.assertFalse(MinStExactPhysicalModel.constraintSatisfied(forbidden, left, right));
		PlacementState different = new PlacementState(right.execType(), right.output(), right.fType(),
			!right.shapeDependent());
		Assert.assertTrue(MinStExactPhysicalModel.constraintSatisfied(forbidden, left, different));
	}
	...
	public void boundedFixtureVariableEliminationMatchesUnquotientedBruteForce() throws Exception {
		...
		MinStExactCategoricalSolver.Result exact = MinStExactCategoricalSolver.solve(
	...
	public void physicalSelectionAndProjectorPreserveExactReceipts() throws Exception {
		...
		MinStExactPhysicalOptimizer.Result optimized = MinStExactPhysicalOptimizer.optimize(
			model, surface, CAMPAIGN_LIMITS);
		MinStExactCategoricalSolver.Result feasible = optimized.solverResult();
	...
	private static BruteForce bruteForce(MinStExactPhysicalModel model,
		List<MinStExactCategoricalSolver.Factor> factors, int index, List<Integer> assignment) {

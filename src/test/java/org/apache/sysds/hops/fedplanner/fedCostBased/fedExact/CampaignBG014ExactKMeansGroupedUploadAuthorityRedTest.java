...
public class CampaignBG014ExactKMeansGroupedUploadAuthorityRedTest {
	@Test
	public void kmeansExactCliCompilationRetainsSelectedFoutAuthority() throws Exception {
		...
		Path script = Path.of("target/g014-exact-kmeans-cli.dml");
		Path config = Path.of("target/g014-exact-kmeans-cli.xml");
		Path x1 = Path.of("target/g014-exact-kmeans-x-1.data");
		Path x2 = Path.of("target/g014-exact-kmeans-x-2.data");
		...
		Files.writeString(script, String.join("\n",
			...
			"write(Y, \"target/g014-exact-kmeans-cli.csv\", format=\"csv\")", ""));
		...
		try {
			...
			ExactCliMetadataFixture.delete(x1, x2);

...
public class ExactAnchorRelocationIdentityRedTest {
	...
	public void equalFTypeAtDifferentAnchorStillRequiresExactUpload() {
		...
		Assert.assertFalse("EXACT_DIRECT_FOUT_MUST_REJECT_EQUAL_FTYPE_AT_DIFFERENT_ANCHOR",
			ExactPhysicalModel.directFoutSatisfied(graph, graph.node(PRODUCER).orElseThrow(),
	...
	public void equalFTypeAtExactSameAnchorSuppressesRedundantUpload() {
		...
		Assert.assertTrue("EXACT_DIRECT_FOUT_MUST_ACCEPT_EXACT_SAME_ANCHOR",
			ExactPhysicalModel.directFoutSatisfied(graph, graph.node(PRODUCER).orElseThrow(),
				CONSUMER, 0, FED_ROW, FType.ROW, selected));
	}
	...
	public void physicalCostAndProjectionRetainTheChosenAnchorEmissionIdentity() throws Exception {
		...
		for(DurableAnchorKey anchor : anchors) {
			...
			List<ExactCategoricalSolver.Factor> factors = new java.util.ArrayList<>(model.hardFactors());
			...
			ExactPhysicalSelection selected = ExactPhysicalSelection.create(model,
				new ExactPhysicalOptimizer.Result(solved, objectiveBits,
					surface.contributionFingerprint()));
			var projected = ExactPhysicalPlacementProjector.project(selected);
			ExactPhysicalModel.Alternative forced = selected.alternativesInDecisionOrder()
			...
			Assert.assertEquals("one source must be direct on the chosen pool", 1L,
				forced.inputAuthorities().stream().filter(authority -> authority.kind()
					== ExactPhysicalModel.InputAuthorityKind.DIRECT_FOUT).count());
	...
	private static DMLProgram compileTwoFederatedSourceFixture() throws Exception {
		...
		String script = "A=federated(addresses=list(\"localhost:1234/A1\",\"localhost:1235/A2\"),"
			...
			+ "ranges=" + ranges + ");\n"
			+ "C=A+B;\nwrite(C,\"/tmp/g014-exact-two-pool-C\",format=\"binary\");\n";

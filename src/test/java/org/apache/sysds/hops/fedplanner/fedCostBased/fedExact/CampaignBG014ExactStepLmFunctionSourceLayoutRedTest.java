...
public class CampaignBG014ExactStepLmFunctionSourceLayoutRedTest {
	@Test
	public void stepLmExactCliCompilationAcceptsMultipleSourceLayouts() throws Exception {
		...
		Path script = Path.of("target/g014-exact-steplm-cli.dml");
		Path config = Path.of("target/g014-exact-steplm-cli.xml");
		Path x = Path.of("target/g014-exact-steplm-x.data");
		Path y = Path.of("target/g014-exact-steplm-y.data");
		...
		Files.writeString(script, String.join("\n",
			...
			"",
			"write(B, \"target/g014-exact-steplm-cli.csv\", format=\"csv\")", ""));

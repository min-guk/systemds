...
/** Regression: the selected Exact physical representative must survive projection verbatim. */
public class ExactProjectionAuthorityPreservationRedTest {
	@Test
	...
	private static int assertPreserved(String fixture, PlacementAnalysis analysis,
		...
		ExactPlacementInput projected = ExactPhysicalPlacementProjector.project(selected);
	...
	private static DMLProgram compileLargeFederatedCandidate() throws Exception {
		String script = "X=federated(addresses=list(\"localhost:1234/X1\",\"localhost:1235/X2\"),"
			...
			+ "Y=X+1;\nwrite(Y,\"target/exact-authority-preservation\",format=\"binary\");\n";

...
import org.apache.sysds.test.functions.federated.paramserv.ParamServTestUtils;
...
public class FederatedCNNParamServPlanningTest extends AutomatedTestBase {
	private static final Log LOG = LogFactory.getLog(FederatedCNNParamServPlanningTest.class.getName());

	private final static String TEST_DIR = "functions/federated/fedplanning/";
	private final static String TEST_NAME = "CNN_network";
	private final static String TEST_CLASS_DIR = TEST_DIR + FederatedCNNParamServPlanningTest.class.getSimpleName()
			+ "/";
	private static File TEST_CONF_FILE;

	private final static int blocksize = 1024;
	public final int rows = 100;
	public final int C = 1, Hin = 28, Win = 28;
	public final int epochs = 1;
	public final int batch_size = 10;
	public final double eta = 0.01;
	public final int workers = 2;
	public final String utype = "BSP";
	public final String freq = "EPOCH";
	public final String mode = "LOCAL";
	...
	@Ignore
	@Test
	public void runCNNParamServFOUTTest() {
		runTestWithConfig("SystemDS-config-fout.xml", null);
	}

	@Test
	public void runCNNParamServHeuristicTest() {
		runTestWithConfig("SystemDS-config-heuristic.xml", null);
	}

	@Ignore
	@Test
	public void runCNNParamServCostBasedTestPrivate() {
		runTestWithConfig("SystemDS-config-cost-based.xml", "private");
	}

	@Test
	public void runCNNParamServCostBasedTestPrivateAggregate() {
		runTestWithConfig("SystemDS-config-cost-based.xml", "private-aggregate");
	}

	@Ignore
	@Test
	public void runCNNParamServCostBasedTestPublic() {
	...
	private void writeInputMatrices(String privacyConstraints) {
		double[][] features = ParamServTestUtils.generateFeatures("CNN", rows, C, Hin, Win);
		double[][] labels = ParamServTestUtils.generateLabels("CNN", rows, 10, C * Hin * Win, features);
	...
	private void loadAndRunTest(String[] expectedHeavyHitters, String testName, String privacyConstraints) {
		...
		try {
			...
			programArgs = new String[] { "-stats", "-nvargs",
					...
					"C=" + C, "Hin=" + Hin, "Win=" + Win, "Z=" + output("Z") };

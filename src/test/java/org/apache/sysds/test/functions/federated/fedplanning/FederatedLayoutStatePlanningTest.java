...
/** End-to-end ROW/COL state witnesses for append and reshape planner rules. */
@net.jcip.annotations.NotThreadSafe
public class FederatedLayoutStatePlanningTest extends AutomatedTestBase {
	private static final String TEST_DIR = "functions/privacy/fedplanning/";
	private static final String TEST_NAME = "FederatedLayoutStatePlanningTest";
	...
	public void setUp() {
		...
		addTestConfiguration(TEST_NAME, new TestConfiguration(TEST_CLASS_DIR, TEST_NAME,
			new String[] {"RR", "RC", "CR", "CC", "SR", "SC"}));
	}

	@Test
	public void fedAllExecutesRowAndColumnLayoutTransitions() {
		getAndLoadTestConfiguration(TEST_NAME);
		double[][] full = getRandomMatrix(ROWS, COLS, -5, 5, 1, 217L);
		writePublicMatrix("X", full);
		writePublicMatrix("R1", rows(full, 0, ROWS / 2));
		writePublicMatrix("R2", rows(full, ROWS / 2, ROWS));
		writePublicMatrix("C1", cols(full, 0, COLS / 2));
		writePublicMatrix("C2", cols(full, COLS / 2, COLS));
		...
		try {
			...
			programArgs = new String[] {"-stats", "100", "-nvargs",
				"R1=" + TestUtils.federatedAddress(port1, input("R1")),
				"R2=" + TestUtils.federatedAddress(port2, input("R2")),
				"C1=" + TestUtils.federatedAddress(port1, input("C1")),
				"C2=" + TestUtils.federatedAddress(port2, input("C2")),
				"r=" + ROWS, "c=" + COLS,
				"RR=" + output("RR"), "RC=" + output("RC"),
				"CR=" + output("CR"), "CC=" + output("CC"),
				"SR=" + output("SR"), "SC=" + output("SC")};
			runTest(true, false, null, -1);

			fullDMLScriptName = home + TEST_NAME + "Reference.dml";
			programArgs = new String[] {"-nvargs", "X=" + input("X"),
				"RR=" + expected("RR"), "RC=" + expected("RC"),
				"CR=" + expected("CR"), "CC=" + expected("CC"),
				"SR=" + expected("SR"), "SC=" + expected("SC")};
			runTest(true, false, null, -1);

			compareResults(1e-9);
			if(!ExactPhysicalForcedStateAudit.isActive()) {
				Assert.assertTrue("Expected federated append execution",
					heavyHittersContainsString("fed_append"));
				Assert.assertTrue("Expected federated reshape execution",

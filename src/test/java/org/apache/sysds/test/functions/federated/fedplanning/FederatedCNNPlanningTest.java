...
public class FederatedCNNPlanningTest extends AutomatedTestBase {
	...
	public final int rows = 200;  // Smaller for test
	public final int cols = 784; // 28*28 for MNIST-like data
	public final int classes = 10;
	...
	private void writeInputMatrices(String privacyConstraints){
		...
		writeStandardRowFedMatrix("Xt1", 85, privacyConstraints);
		writeStandardRowFedMatrix("Xt2", 95, privacyConstraints);
		writeCategoricalVector("Y", 44, privacyConstraints);
		writeCategoricalVector("Yt", 54, privacyConstraints);
	}

	private void writeCategoricalVector(String matrixName, long seed, String privacyConstraints){
		double[][] matrix = getRandomMatrix(rows, classes, 0, 1, 1, seed);
		...
		for(int i = 0; i < rows; i++) {
			...
			for(int j = 0; j < classes; j++) {
				matrix[i][j] = (j == maxIdx) ? 1.0 : 0.0;
			}
		}
		MatrixCharacteristics mc = new MatrixCharacteristics(rows, classes, blocksize, (long) rows * classes);
	...
	private void writeStandardRowFedMatrix(String matrixName, long seed, String privacyConstraints){
		// Generate MNIST-like data that can be reshaped to 28x28
		double[][] matrix = getRandomMatrix(rows / 2, cols, 0, 255, 1, seed);
	...
	private void loadAndRunTest(String[] expectedHeavyHitters, String testName, String privacyConstraints){
		...
		try {
			...
			programArgs = new String[] { "-stats", "-nvargs",
				...
				"Xt1=" + TestUtils.federatedAddress(port3, input("Xt1")),
				"Xt2=" + TestUtils.federatedAddress(port4, input("Xt2")),
				"Y=" + input("Y"), "Yt=" + input("Yt"), 

...
/** Direct ROW/COL witnesses for aligned, crossing, and full aggregate-unary operations. */
@net.jcip.annotations.NotThreadSafe
public class FederatedAggregateUnaryLayoutPlanningTest extends AutomatedTestBase {
	private static final String TEST_DIR = "functions/privacy/fedplanning/";
	private static final String TEST_NAME = "FederatedAggregateUnaryLayoutPlanningTest";
	private static final String TEST_CLASS_DIR = TEST_DIR + TEST_NAME + "/";
	private static final String[] OUTPUTS = {
		"RowRows", "RowCols", "RowAll.scalar", "ColRows", "ColCols", "ColAll.scalar"
	};
	...
	@Test
	public void fedAllExecutesAggregateUnaryForRowAndCol() {
		getAndLoadTestConfiguration(TEST_NAME);
		double[][] x = getRandomMatrix(ROWS, COLS, 0.1, 1.0, 1, 557L);
		...
		try {
			...
			if(!ExactPhysicalForcedStateAudit.isActive()) {
				Assert.assertTrue("Expected federated row aggregate",
					heavyHittersContainsString("fed_uark+"));
				Assert.assertTrue("Expected federated column aggregate",
					heavyHittersContainsString("fed_uack+"));
				Assert.assertTrue("Expected federated full aggregate",
					heavyHittersContainsString("fed_uak+"));
			}
		}
		finally {
	...
	private String[] federatedArgs(int port1, int port2) {
		return new String[] {"-stats", "100", "-nvargs",
			...
			"r=" + ROWS, "c=" + COLS,
			"RowRows=" + output("RowRows"), "RowCols=" + output("RowCols"),
			"RowAll=" + output("RowAll"), "ColRows=" + output("ColRows"),
			"ColCols=" + output("ColCols"), "ColAll=" + output("ColAll")};
	}

	private String[] referenceArgs() {
		return new String[] {"-nvargs", "X=" + input("X"),
			"RowRows=" + expected("RowRows"), "RowCols=" + expected("RowCols"),
			"RowAll=" + expected("RowAll"), "ColRows=" + expected("ColRows"),
			"ColCols=" + expected("ColCols"), "ColAll=" + expected("ColAll")};
	}

...
/** Direct ROW/COL witnesses for aggregate-binary matrix multiplication. */
@net.jcip.annotations.NotThreadSafe
public class FederatedAggregateBinaryLayoutPlanningTest extends AutomatedTestBase {
	private static final String TEST_DIR = "functions/privacy/fedplanning/";
	private static final String TEST_NAME = "FederatedAggregateBinaryLayoutPlanningTest";
	private static final String TEST_CLASS_DIR = TEST_DIR + TEST_NAME + "/";
	private static final String[] OUTPUTS = {"RowMM", "ColMV", "LeftVM"};
	...
	@Test
	public void fedAllExecutesAggregateBinaryForRowAndCol() {
		getAndLoadTestConfiguration(TEST_NAME);
		double[][] x = getRandomMatrix(ROWS, INNER, 0.1, 1.0, 1, 521L);
		double[][] b = getRandomMatrix(INNER, COLS, 0.1, 1.0, 1, 523L);
		double[][] v = getRandomMatrix(INNER, 1, 0.1, 1.0, 1, 541L);
		double[][] left = getRandomMatrix(1, ROWS, 0.1, 1.0, 1, 547L);

		writePublicMatrix("X", x);
		writePublicMatrix("B", b);
		writePublicMatrix("V", v);
		writePublicMatrix("Left", left);
		writePublicMatrix("XR1", rows(x, 0, ROWS / 2));
		writePublicMatrix("XR2", rows(x, ROWS / 2, ROWS));
		writePublicMatrix("XC1", cols(x, 0, INNER / 2));
		writePublicMatrix("XC2", cols(x, INNER / 2, INNER));
		...
		try {
			...
			if(!ExactPhysicalForcedStateAudit.isActive())
				Assert.assertTrue("Expected direct federated aggregate binary",
	...
	private String[] referenceArgs() {
		return new String[] {"-nvargs", "X=" + input("X"), "B=" + input("B"),

...
/** Direct ROW/COL runtime witnesses for weighted cross entropy and weighted unary matrix multiplication. */
@net.jcip.annotations.NotThreadSafe
public class FederatedWCeMMWUMMLayoutPlanningTest extends AutomatedTestBase {
	private static final String TEST_DIR = "functions/privacy/fedplanning/";
	private static final String TEST_NAME = "FederatedWCeMMWUMMLayoutPlanningTest";
	private static final String TEST_CLASS_DIR = TEST_DIR + TEST_NAME + "/";
	private static final String[] OUTPUTS = {"CeRow", "CeCol", "UmmRow", "UmmCol"};
	...
	@Test
	public void fedAllExecutesWCeMMAndWUMMForRowAndCol() {
		getAndLoadTestConfiguration(TEST_NAME);
		double[][] x = getRandomMatrix(ROWS, COLS, 0.25, 1.25, 1, 443L);
		double[][] u = getRandomMatrix(ROWS, RANK, 0.1, 0.4, 1, 449L);
		double[][] v = getRandomMatrix(COLS, RANK, 0.1, 0.4, 1, 457L);
		...
		try {
			...
			if(!ExactPhysicalForcedStateAudit.isActive()) {
				Assert.assertTrue("Expected direct federated weighted cross entropy",
					heavyHittersContainsString("fed_wcemm"));
				Assert.assertTrue("Expected direct federated weighted unary matrix multiplication",
					heavyHittersContainsString("fed_wumm"));
			}
		}
		finally {
	...
	private String[] federatedArgs(int port1, int port2) {
		return new String[] {"-stats", "100", "-nvargs",
			...
			"U=" + input("U"), "V=" + input("V"),
			"r=" + ROWS, "c=" + COLS,
			"CeRow=" + output("CeRow"), "CeCol=" + output("CeCol"),
			"UmmRow=" + output("UmmRow"), "UmmCol=" + output("UmmCol")};
	}

	private String[] referenceArgs() {
		return new String[] {"-nvargs", "X=" + input("X"), "U=" + input("U"), "V=" + input("V"),
			"CeRow=" + expected("CeRow"), "CeCol=" + expected("CeCol"),
			"UmmRow=" + expected("UmmRow"), "UmmCol=" + expected("UmmCol")};
	}

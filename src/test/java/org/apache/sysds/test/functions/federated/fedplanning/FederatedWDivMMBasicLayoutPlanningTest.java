...
/**
 * Direct runtime witnesses for MULT_BASIC WDivMM over ROW- and COL-partitioned
 * federated inputs. The column count intentionally exceeds the block size: the
 * dynamic algebraic rewrite only selects the BASIC kernel for a non-vector
 * weight matrix that spans more than one column block.
 */
@net.jcip.annotations.NotThreadSafe
public class FederatedWDivMMBasicLayoutPlanningTest extends AutomatedTestBase {
	private static final String TEST_DIR = "functions/privacy/fedplanning/";
	private static final String TEST_NAME = "FederatedWDivMMBasicLayoutPlanningTest";
	private static final String TEST_CLASS_DIR = TEST_DIR + TEST_NAME + "/";
	private static final String[] OUTPUTS = {"BasicRow", "BasicCol"};
	...
	@Test
	public void fedAllExecutesBasicWeightedDivMMForRowAndCol() {
		getAndLoadTestConfiguration(TEST_NAME);
		double[][] x = getRandomMatrix(ROWS, COLS, 1, 4, 1, 409L);
		double[][] u = getRandomMatrix(ROWS, RANK, 1, 3, 1, 419L);
		double[][] v = getRandomMatrix(COLS, RANK, 1, 3, 1, 421L);
		...
		try {
			...
			if(!ExactPhysicalForcedStateAudit.isActive())
				Assert.assertTrue("Expected direct BASIC federated WDivMM",

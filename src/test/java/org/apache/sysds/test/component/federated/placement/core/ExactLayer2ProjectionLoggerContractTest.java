...
public class ExactLayer2ProjectionLoggerContractTest {
	@Test
	public void selectedReceiptsAreABijectiveLegalProjectionOfTheNeutralGraph() throws Exception {
		...
		try {
			...
			Assert.fail("EXACT_LAYER2_SELECT_BOUNDARY_MISSING");
			return;
		}
		...
		for(String fixture : List.of("B-01", "B-07", "B-09", "B-16")) {
			...
			ExactPlacementInput input = new FederatedPlanExact()
				.rewriteProgram(program, null, null, analysis);
			ExactPlacementAdapter.Selection selection = invoke(select, analysis, input);
			...
			List<CompiledHopKey> actual = selection.selectedReceipts().stream()
				.map(ExactPlacementAdapter.SelectedReceipt::planningKey).toList();
			...
			for(ExactPlacementAdapter.SelectedReceipt receipt : selection.selectedReceipts()) {
	...
	private static ExactPlacementAdapter.Selection invoke(Method select, PlacementAnalysis analysis,
		ExactPlacementInput input) throws Exception {
		try {
			return (ExactPlacementAdapter.Selection) select.invoke(new ExactPlacementAdapter(), analysis, input);
		}
	...
	private static void assertGraphFree(Type type) {
		...
		Assert.assertFalse("EXACT_LAYER2_GRAPH_TYPED_LOGGER:" + name,

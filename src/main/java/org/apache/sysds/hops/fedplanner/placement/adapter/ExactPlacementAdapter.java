...
/** Observational adapter for an already selected, owner-bound Exact result. */
public final class ExactPlacementAdapter {
	...
	public record Selection(PlacementAnalysis analysis, ProducerReceipt producer,
		String analysisFingerprint, long objectiveBits, List<SelectedReceipt> selectedReceipts) {
		public Selection {
	...
	public Selection select(PlacementAnalysis requested, ExactPlacementInput ownerBound) {
		...
		return new Selection(requested, producer, requested.analysisFingerprint(), producer.objectiveBits(),
			receipts);
	}
}

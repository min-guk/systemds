...
import org.apache.sysds.hops.fedplanner.fedCostBased.fedMinSTCut.MinStExactCategoricalSolver.Factor;
import org.apache.sysds.hops.fedplanner.fedCostBased.fedMinSTCut.MinStExactCategoricalSolver.Variable;
import org.apache.sysds.hops.fedplanner.fedCostBased.fedMinSTCut.MinStExactPhysicalModel.Alternative;
import org.apache.sysds.hops.fedplanner.fedCostBased.fedMinSTCut.MinStExactPhysicalModel.DecisionDomain;
...
public class FederatedPlanMinSTCut extends AFederatedPlanner {
	 ...
	 */
	private static void tracePhysicalSelection(MinStExactPhysicalModel model,
		MinStExactCostFactsProducer.PhysicalCostSurface surface,
		MinStExactPhysicalSelection selection) {
		if(!FederatedPlannerTrace.isEnabled())
			return;
		List<Integer> assignment = selection.assignmentInDecisionOrder();
		MinStExactCategoricalSolver.Statistics statistics = selection.statistics();
		FederatedPlannerTrace.logGlobal("MinST-PhysicalOptimize", String.format(Locale.ROOT,
		...
		for(int index = 0; index < model.domains().size(); index++) {
			...
			FederatedPlannerTrace.log(hop, "MinST-PhysicalSelect", String.format(Locale.ROOT,
			...
			for(int alternativeIndex = 0; alternativeIndex < domain.alternatives().size(); alternativeIndex++) {
				...
				FederatedPlannerTrace.log(hop, "MinST-PhysicalAlternative", String.format(Locale.ROOT,
			...
			if(omitted > 0)
				FederatedPlannerTrace.log(hop, "MinST-PhysicalAlternativeSummary",
		...
		FederatedPlannerTrace.logGlobal("MinST-PhysicalComplete", String.format(Locale.ROOT,
	...
	private static double fixedOthersIncidentCost(List<Factor> factors, Variable variable,
		...
		for(Factor factor : factors) {
			...
			for(int localIndex = 0; localIndex < local.length; localIndex++) {
				...
				if(globalIndex == null)
					throw new IllegalArgumentException("MINST_TRACE_FOREIGN_VARIABLE");
				local[localIndex] = scoped == variable ? variableValue : selectedAssignment.get(globalIndex);
			}
			double cost = factor.cost(local);
			if(cost == Double.POSITIVE_INFINITY)
				return cost;
			sum.addBits(Double.doubleToRawLongBits(cost), "MINST_TRACE_INCIDENT_COST_INVALID",
				"MINST_TRACE_INCIDENT_TOTAL_INVALID");
		}
		return Double.longBitsToDouble(sum.totalBits("MINST_TRACE_INCIDENT_TOTAL_INVALID"));
	}

...
public class QuaternaryOp extends MultiThreadedHop
{
			...
			switch( _op ) {
				case WSLOSS: {
					...
					if( et == ExecType.CP )
						...
						constructFederatedLopsWeightedSquaredLoss(wtype);
				...
				case WSIGMOID:{
					...
					if( et == ExecType.CP )
						...
						constructFederatedLopsWeightedSigmoid(wtype);
					else if( et == ExecType.SPARK )
				...
				case WDIVMM:{
					...
					if( et == ExecType.CP )
						...
						constructFederatedLopsWeightedDivMM(wtype);
					else if( et == ExecType.SPARK )
				...
				case WCEMM:{
					...
					if( et == ExecType.CP )
						...
						constructFederatedLopsWeightedCeMM(wtype);
					else if( et == ExecType.SPARK )
				...
				case WUMM:{
					...
					if( et == ExecType.CP )
						...
						constructFederatedLopsWeightedUMM(wtype);
					else if( et == ExecType.SPARK )
	...
	public boolean supportsFederatedExecution() {
		switch( _op ) {
			case WSLOSS:
			case WSIGMOID:
			case WCEMM:
			...
			case WDIVMM: {
				WDivMMType wtype = checkWDivMMType();
				return wtype != null && (wtype.isBasic() || wtype.isLeft() || wtype.isRight());
			}
			default:
				return false;
		}
	...
	private void constructFederatedLopsWeightedSquaredLoss(WeightsType wtype)
	{
		WeightedSquaredLoss wsloss = new WeightedSquaredLoss(
			getInput().get(0).constructLops(),
			getInput().get(1).constructLops(),
			getInput().get(2).constructLops(),
			getInput().get(3).constructLops(),
			getDataType(), getValueType(), wtype, ExecType.FED);
		wsloss.setNumThreads(OptimizerUtils.getConstrainedNumThreads(_maxNumThreads));
		setOutputDimensions(wsloss);
		setLineNumbers(wsloss);
		setLops(wsloss);
	}
	...
	private void constructFederatedLopsWeightedSigmoid(WSigmoidType wtype) {
		WeightedSigmoid wsig = new WeightedSigmoid(
			getInput().get(0).constructLops(),
			getInput().get(1).constructLops(),
			getInput().get(2).constructLops(),
			getDataType(), getValueType(), wtype, ExecType.FED);
		wsig.setNumThreads(OptimizerUtils.getConstrainedNumThreads(_maxNumThreads));
		setOutputDimensions(wsig);
		setLineNumbers(wsig);
		setLops(wsig);
	}
	...
	private void constructFederatedLopsWeightedDivMM(WDivMMType wtype)
	{
		WeightedDivMM wdiv = new WeightedDivMM(
			getInput().get(0).constructLops(),
			getInput().get(1).constructLops(),
			...
			getInput().get(3).constructLops(),
			getDataType(), getValueType(), wtype, ExecType.FED);
		wdiv.setNumThreads(OptimizerUtils.getConstrainedNumThreads(_maxNumThreads));
		setOutputDimensions(wdiv);
		setLineNumbers(wdiv);
		setLops(wdiv);
	}
	...
	private void constructFederatedLopsWeightedCeMM(WCeMMType wtype)
	{
		WeightedCrossEntropy wcemm = new WeightedCrossEntropy(
			getInput().get(0).constructLops(),
			getInput().get(1).constructLops(),
			...
			getInput().get(3).constructLops(),
			getDataType(), getValueType(), wtype, ExecType.FED);
		wcemm.setNumThreads(OptimizerUtils.getConstrainedNumThreads(_maxNumThreads));
		setOutputDimensions(wcemm);
		setLineNumbers(wcemm);
		setLops(wcemm);
	}
	...
	private void constructFederatedLopsWeightedUMM(WUMMType wtype) {
		OpOp1 uop = _uop!=null ? _uop : _sop==OpOp2.POW ?
			OpOp1.POW2 : OpOp1.MULT2;
		WeightedUnaryMM wumm = new WeightedUnaryMM(
			getInput().get(0).constructLops(),
			getInput().get(1).constructLops(),
			getInput().get(2).constructLops(),
			getDataType(), getValueType(), wtype, uop, ExecType.FED);
		wumm.setNumThreads(OptimizerUtils.getConstrainedNumThreads(_maxNumThreads));
		setOutputDimensions(wumm);
		setLineNumbers(wumm);
		setLops(wumm);
	}

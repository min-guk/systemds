...
public class AggregateBinaryFEDInstruction extends BinaryFEDInstruction {
	...
	public void processInstruction(ExecutionContext ec) {
		...
		rejectLocalBackedFederationMap(input1, mo1);
		rejectLocalBackedFederationMap(input2, mo2);
		if (DEBUG_KMEANS) {
	...
	private void rejectLocalBackedFederationMap(CPOperand operand, MatrixLineagePair input) {
		if (input != null && input.isFederated() && input.getFedMapping() != null
			...
			throw new DMLRuntimeException("FED aggregate binary cannot repair local-backed federated input "
				+ operand.getName() + "; planner must emit an explicit fed_refed/fed_fout relocation. inst="
				+ instString);
	}

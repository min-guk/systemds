...
for (profile in profiles) {
  png(file.path(out, paste0("runtime_", profile, "_interim_270.png")),
  ...
  mtext(paste0("Interim authenticated Docker runtimes — ", profile, " — 270/336 cells"),
...
png(file.path(out, "matched_four_planner_ratios_interim_270.png"),
...
boxplot(ratio.list, names = planners, col = unname(cols[planners]),
        ...
        main = paste0("Four-planner matched cells (n=", length(complete.keys), ") — interim 270/336"),
        outline = TRUE, pch = 16)
...
png(file.path(out, "coverage_by_planner_interim_270.png"),
...
bars <- barplot(counts, col = unname(cols[planners]), ylim = c(0, 92),
                ...
                main = "Current no-duplicate coverage — 270/336 cells")
abline(h = 84, col = "#333333", lty = 2, lwd = 2)
text(bars, counts + 3, labels = paste0(counts, "/84"), font = 2)
mtext("DP, FedAll, Heuristic complete; MinST has 18 successes and 66 cells remaining.",

#!/usr/bin/env python3
"""Build the authenticated, no-duplicate interim 270-cell result table."""
...
def main() -> None:
	...
	registry = read_json(root / "base-completed.json")
	manifest = read_json(root / "campaign-manifest.json")
	marker = read_json(root / "CAMPAIGN_FAILED.json")
	rows: list[dict] = []
	...
	response_by_cell: dict[str, pathlib.Path] = {}
	for response_path in (root / "planners" / "MinST" / "cells").glob("*/response.json"):
		response = read_json(response_path)
		if response.get("success") is True:
			response_by_cell[response["cell"]] = response_path
	current_rows = root / "planners" / "MinST" / "rows.jsonl"
	for line in current_rows.read_text().splitlines():
		if not line.strip():
			continue
		row = json.loads(line)
		response_path = response_by_cell[row["cell"]]
		actual_response_sha = sha256(response_path)
		verified_row(row, row["cell"], actual_response_sha)
		response = read_json(response_path)
		if response.get("teardown_zero_resources") is not True:
			raise ValueError(f"teardown not proven: {row['cell']}")
		rows.append(materialize(row, {
			"source_kind": "successful-prefix-before-MinST-Steplm-failure",
			"campaign_root": str(root),
			"stage": manifest["stage"],
			"systemds_commit": manifest["identity"]["systemds_commit"],
			"systemds_jar_sha256": manifest["identity"]["jar_sha256"],
			"response_path": str(response_path),
		}))
	...
	if len(rows) != 270 or registry["completed_cells"] != 262 or marker["successful_new_cells"] != 8:
		raise ValueError("interim cardinality is not 262 + 8 = 270")

	rows.sort(key=lambda row: (
		PLANNERS.index(row["planner"]), row["workers"],
		WORKLOADS.index(row["workload"]), PROFILES.index(row["profile"])))
	fieldnames = list(rows[0])
	csv_path = out / "authenticated_rows_270.csv"
	...
	summary = {
		...
		"frozen_campaign_marker_sha256": sha256(root / "CAMPAIGN_FAILED.json"),
		"base_registry_sha256": sha256(root / "base-completed.json"),
		"current_rows_sha256": sha256(current_rows),

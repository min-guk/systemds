#!/usr/bin/env python3
"""Build the authenticated, no-duplicate interim 282-cell result table."""
...
def main() -> None:
	...
	failure_marker_path = root / "CAMPAIGN_FAILED.json"
	failure_marker = read_json(failure_marker_path)
	planner_manifest = read_json(root / "planners/MinST/manifest.json")
	rows: list[dict] = []
	...
	new_rows_path = root / "planners/MinST/rows.jsonl"
	if sha256(new_rows_path) != failure_marker["rows_sha256"]:
		raise ValueError("frozen MinST rows hash mismatch")
	new_rows = [json.loads(line) for line in new_rows_path.read_text().splitlines()]
	if len(new_rows) != 8 or failure_marker["successful_new_cells"] != 8:
		raise ValueError("frozen campaign does not contain exactly 8 new successes")
	for row in new_rows:
		bundle = pathlib.Path(row["bundle"])
		response_path = bundle.parents[2] / "response.json"
		response_sha = sha256(response_path)
		verified_row(row, row["cell"], response_sha)
		response = read_json(response_path)
		oracle = read_json(bundle / "semantic_oracle.json")
		scan = read_json(bundle / "scan.json")
		if (response.get("success") is not True
			or response.get("teardown_zero_resources") is not True
			or response.get("coordinator_restart_count") != 0
			or response.get("worker_restart_count") != 0
			or oracle.get("passed") is not True
			or any(scan.get(flag) is not False for flag in
				("error", "fallback", "resource_invalid", "timeout"))):
			raise ValueError(f"new response lifecycle proof failed: {row['cell']}")
		rows.append(materialize(row, {
			"source_kind": "failed-campaign-success-prefix-before-minst-l2svm-2w",
			"campaign_root": str(root),
			"stage": planner_manifest["stage_descriptor"].removesuffix("/stage-descriptor.json"),
			"systemds_commit": planner_manifest["systemds_commit"],
			"systemds_jar_sha256": planner_manifest["systemds_jar_sha256"],
			"response_path": str(response_path),
		}))
	...
	if (len(rows) != 282 or registry["completed_cells"] != 274
		or failure_marker["combined_successful_cells"] != 282
		or failure_marker["remaining_successes_needed"] != 54
		or failure_marker["resume_permitted"] is not False
		or failure_marker["retry_performed"] is not False):
		raise ValueError("interim cardinality/freeze contract is not 282/54")

	rows.sort(key=lambda row: (
		PLANNERS.index(row["planner"]), row["workers"],
		WORKLOADS.index(row["workload"]), PROFILES.index(row["profile"])))
	fieldnames = list(rows[0])
	csv_path = out / "authenticated_rows_282.csv"
	...
	summary = {
		...
		"frozen_campaign_failure_marker_sha256": sha256(failure_marker_path),
		"frozen_campaign_rows_sha256": sha256(new_rows_path),

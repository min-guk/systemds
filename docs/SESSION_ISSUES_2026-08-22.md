...
- **상태**: 구현 완료, Docker compile/runtime 검증 진행중
- **환경/조건**: commit `c792138e7b`, authenticated Docker WAN-light, 2 workers, LOGREG `multiLogReg(maxi=30, maxii=5)`, FedAll/Heuristic exact-policy selector와 first-feasible selector 비교.
- **재현 절차**:
  ```bash
  # 기존 완료 artifact
...
- **해결 계획**:
  ...
  3. 동일 policy rank에서 incident movement를 execution frequency로 가중하고, 여전히 동률이면 native/direct consumer-axis-aligned ROW/COL을 derived/relocating BROADCAST보다 우선한다.
...
- **잔여 이슈**: data-dependent while frequency는 compile-time estimate이므로 runtime actual count와 다를 수 있다. Exact-policy와 cost-based Exact를 용어상 구분해야 한다.

...
**보고 시각:** 2026-08-31 11:46:16 CEST  
...
각 manifest row는 별도 Maven/Surefire JVM에서 실행된다. 11:46:16 CEST 최종 결과:

| Host | 완료 | SUCCESS | TNR | WPI | TRIAGE |
|---|---:|---:|---:|---:|---:|
| `so003` | 285/285 | 72 | 85 | 14 | 114 |
| `so004` | 268/268 | 60 | 95 | 13 | 100 |
| `so006` | 271/271 | 66 | 99 | 11 | 95 |
| **합계** | **824/824 (100.00%)** | **198** | **279** | **38** | **309** |

세 isolated summary가 모두 생성됐다. Infrastructure status는 전부 `PASS`이며
failed JVM, duplicate, missing, unexpected target은 모두 0이다.

- constraint 적용·충족: 507개 = `SUCCESS` 198 + `TRIAGE` 309
- runtime capability: 3,539/3,539 `SUCCESS`
- `TARGET_NOT_REACHED` 279개는 workload FED instruction은 실행됐지만 forcing
  occurrence가 재구성되지 않은 경우다. Physical failure로 분류하지 않는다.
- `WHOLE_PROGRAM_INFEASIBLE` 38개는 실행 전 hard constraint에서 차단됐다.

Persistent TNR 279개와 semantic manifest의 target-ID 집합은 정확히 같다.
Missing/unexpected/duplicate ID는 0/0/0이다.
...
Isolated watcher는 정상 완료·종료했고 semantic watcher 세 개가 실행 중이다.
11:46:16 CEST semantic 진행률은 52/279이다.

| 결과 | 수 |
|---|---:|
| `SUCCESS` | 15 |
| `TARGET_NOT_REACHED` | 34 |
| `REPLAY_IDENTITY_AMBIGUOUS` | 3 |

Semantic watcher PID 3778801(`so003`), 193806(`so004`),
2972640(`so006`)은 모두 alive다. Capability-aware detached finalizer도 alive다.
...
Isolated 완료 checkpoint:

```text
audit-results/fed-space-forced-normalized-v2-20260831T053209Z-isolated-complete-20260831T114614+0200/
SHA256SUMS.txt SHA-256:
  45ab5c378fcab3bf76536c6c6325337eaf2195d46e96ec931ad75792163cabcb
```
...
1. semantic v3 279/279 완료 및 세 summary의 infrastructure PASS 검증
2. semantic unique/ambiguous/not-emitted 분류 확정

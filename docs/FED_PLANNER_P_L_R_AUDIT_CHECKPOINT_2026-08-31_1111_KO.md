...
**보고 시각:** 2026-08-31 11:11:36 CEST  
...
각 manifest row는 별도 Maven/Surefire JVM에서 실행된다. 11:11:36 CEST 진행률:

| Host | 완료 | SUCCESS | TNR | WPI | TRIAGE |
|---|---:|---:|---:|---:|---:|
| `so003` | 175/285 | 61 | 72 | 13 | 29 |
| `so004` | 154/268 | 53 | 77 | 13 | 11 |
| `so006` | 149/271 | 56 | 85 | 6 | 2 |
| **합계** | **478/824 (58.01%)** | **170** | **234** | **32** | **42** |

현재 결과는 진행 중 snapshot이다. Isolated summary가 생성되기 전에는 outcome
cardinality를 최종값으로 사용하지 않는다.

11:11 snapshot의 outcome별 evidence는 다음과 같다.

- `SUCCESS` 170개: constraint 170/170 충족, capability 832/832 `SUCCESS`
- `TARGET_NOT_REACHED` 234개: constraint 미적용, capability 1,745/1,745 `SUCCESS`
- `FAILURE_REQUIRES_TRIAGE` 42개: constraint 42/42 충족; capability가 기록된
  36개 target의 47개 instruction은 전부 `SUCCESS`
- `WHOLE_PROGRAM_INFEASIBLE` 32개: 실행 전 hard constraint에서 차단되어
  runtime-capability row 없음

현재까지 isolated runtime-capability `FAILURE` 증거는 0이다. 따라서 TNR은
물리 실패가 아니라 replay occurrence 미도달이며 semantic v3 대상이다.
...
현재 isolated progress checkpoint:

```text
audit-results/fed-space-forced-normalized-v2-20260831T053209Z-isolated-progress-20260831T111132+0200/
SHA256SUMS.txt SHA-256:
  ccd65334919999ec798cd098710883db702abb7c8c39557461168d0723d73868
```

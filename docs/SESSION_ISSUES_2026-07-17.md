...
- **Observed symptom**: `FederatedPlannerDpRewireTransTable` mutates clone, transient, loop, function, and additional-root carriers through legacy analysis-blind entry points. It has no direct typed boundary that proves an exact `PlacementAnalysis`, occurrence order, clone/original association, SAME_ORIGIN constraint, and recompile exclusion before any topology work.
- **Root cause**: The rewire owner predates the immutable neutral placement analysis. Its live caller does not supply `PlacementAnalysis`, so replacing the live walk would require a protected enumerator edit and is outside Slice 4.
- **Resolution summary**: Added a direct non-reflective RED requiring the Task 4-approved additive `inspectExact(RewireRequest)` projection. B-09 requires exactly one analysis-owned clone/origin receipt, exact SAME_ORIGIN and RECOMPILE_CP_FOUT object identities, immutable clone-to-original Hop IDs, ordered normalized identities, and no fabricated legacy-only additional roots. B-05 requires an empty clone receipt/map and unchanged original occurrences. Success and copied/reordered/foreign/substituted-owner rejection paths freeze the program fingerprint, complete occurrence/key/Hop order, node/constraint/exclusion identities, Hop parent/input topology, memo object and full legal presence universe, selected root/child plan identities, raw cumulative/self/forwarding bits, exec/FType, child-edge identity/order/multiplicity, and exact structural counters.
- **Modified files**:
  - `src/test/java/org/apache/sysds/hops/fedplanner/fedCostBased/fedDp/CampaignBDpRewireOwnerContractTest.java`
  - `docs/SESSION_ISSUES_2026-07-17.md`
- **Verification**: Static source/scope checks, direct typed-symbol scan, forbidden reflection/Object-erasure scan, `git diff --check`, and zero production delta. No Maven or generated command is run in the RED task.
...
- **Potential regression risk**: An implementation could accept an equal copied occurrence list, reconstruct a second clone universe, match SAME_ORIGIN/exclusion by value rather than analysis-owned identity, sort order, fabricate additional roots, or mutate memo/plan/Hop topology while returning numerically equal costs. Detect with the direct identity checks, raw-bit snapshots, immutable receipt assertions, complete before/after snapshots, and exact closure/frozen-suite verification.

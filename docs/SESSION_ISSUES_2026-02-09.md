...
## 이슈 3: DP에서 OPTIONAL 완화 후 FED 가능성 게이트가 과완화되어 REQUIRED 입력 검증이 누락될 위험

- **상태**: 해결
- **환경/조건**:
  - Repository: `/tmp/systemds-wt-dp-align`
...
## 이슈 4: `FederatedPlanTReadWriteConsistencyTest`의 MinST hard-edge 기대와 현재 MinST 정책 불일치
...
## 이슈 5: `fromFTypes` planned-fed null 허용이 일부 보조 경로에 미반영되어 FED 가능성 판정이 불일치
...
- **잔여 이슈**:
  - MinST hard-edge 계약 이슈는 별도(이슈 4)로 계속 추적.
- **잠재 회귀 위험**:
...
## 이슈 6: DP가 파생 `FED/FOUT`를 선택해도 rewrite 단계에서 `federatedOutputDerived`가 누락됨
...
## 이슈 7: DP 열거기 `enumerateHop` 시그니처 확장으로 reflection 기반 테스트가 `NoSuchMethod`로 붕괴
...
## 이슈 8: 공통 upload-hint API가 `hopId==0`을 무효 처리해 힌트를 누락함
...
## 이슈 9: DP에서 `CP/FOUT` 후보를 refed-전용 게이트로 막아 root 업로드가 불가능해짐
...
## 이슈 10: DP에서 OPTIONAL 입력의 LOUT->FED forwarding 비용을 0으로 처리해 FED 플랜 비용이 과소평가됨

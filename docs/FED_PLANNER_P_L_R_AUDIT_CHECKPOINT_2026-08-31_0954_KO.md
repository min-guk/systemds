...
- provisional TRIAGE 300개는 모두 runtime-capability row를 남겼고, 584개
  instruction outcome이 전부 `SUCCESS`였다. 따라서 현재 증거는 물리 실패가 아니라
  forced alternative가 workload heavy-hitter assertion을 바꾼 경우를 가리킨다.
  다만 authoritative 분류는 isolated re-entry 이후 확정한다.

09:54 checkpoint에는 primary 554개가 남았다. 최근 처리율만 단순 외삽하면
10:10--10:20 CEST 전후 종료 가능성이 있으나, workload별 실행시간 변화를
무시한 **대략적 추정**이다. 이후 isolated 및 semantic 단계 시간이 추가된다.
...
```text
aggregate precedence/coverage unit tests: 3/3 PASS
remote compact-copy smoke:                PASS
real aggregation checksum smoke:          PASS
...
Finalizer는 완료 후 세 호스트에서 summary, run manifest, forced-state result,
runtime-capability row와 retry plan을 수집하고, stage coverage를 검증한 뒤 최종
JSONL/CSV/Markdown 및 artifact checksum을 생성한다.
...
| `...073941Z` | 최초 detached tmux 수정본 | 아니오; capability evidence 보존본으로 교체 |
| `...074722Z` | runtime-capability row 보존·triage 분류 추가 | **현재 사용 중** |

...
def verify(matrix_dir, catalog, evaluation, *, expected_conditions=None, expected_cell=None,
           full_in_scope=False, ready_in_scope=False):
    ...
    failures = []
    verified_complete = 0
    for listed_row in sorted(listed, key=lambda row: row["cell"]):
        cell = listed_row["cell"]
        row = expected[cell]
        receipt_path = matrix_dir / cell / "receipt.json"
        if not receipt_path.is_file():
            failures.append({"cell": cell, "reason": "MISSING_RECEIPT"})
            continue
        ...
        if (receipt.get("schema") != "current-p-matrix-cell-v1" or
                receipt.get("cell") != cell or status != listed_row.get("status") or
                receipt.get("sourceTreeSha256") != manifest.get("sourceTreeSha256") or
                receipt.get("classTreeSha256") != manifest.get("classTreeSha256") or
                receipt.get("runnerSha256") != manifest.get("runnerSha256") or
                receipt.get("evaluationRoot") != str(evaluation) or
                receipt.get("catalogSha256") != manifest["catalogSha256"] or
